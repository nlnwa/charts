...
SUBJECT="/C=NO/CN=${HOSTNAME}"

# Generate CA signing key
openssl genrsa -out ca.key 2048

# Generate CA certificate and self-sign it
openssl req -new -x509 -days 365 -key ca.key -subj "/C=NO/CN=${AUTHORITY}" -out ca.crt

# Generate certificate signing request and private key for server certificate
openssl req -newkey rsa:2048 -nodes -keyout tls.key -subj "/CN=${HOSTNAME}" -out tls.csr

# Sign server certificate with (self-signed) CA certificate and key
openssl x509 -req -extfile <(printf "subjectAltName=DNS:${HOSTNAME},DNS:localhost") -days 365 -in tls.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out tls.crt
